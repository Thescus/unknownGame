...
void AMyProjectCharacter::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	...
	//Raycast
	InputComponent->BindAction("Action", IE_Pressed, this, &AMyProjectCharacter::OnAction);
}

void AMyProjectCharacter::OnAction()
{
	//declare unt init some vars
	const float ACTION_DISTANCE = 500.f;
	FVector StartTrace, EndTrace;
	ECollisionChannel CollisionChannel = ECC_Pawn;
	ACharacter* characterController = UGameplayStatics::GetPlayerCharacter(GetWorld(), 0);
	
	AActor* ActorToIgnore = characterController;
	FCollisionQueryParams TraceParams(FName(TEXT("testTrace")), true, ActorToIgnore);
	TraceParams.bTraceComplex = true;
	TraceParams.bReturnPhysicalMaterial = false;
	StartTrace = characterController->GetActorLocation() + FVector(0.f, 0.f, 65.0f);
	EndTrace = StartTrace + (GetControlRotation().Vector() * 256);

	//The thing that tells us what we hit
	FHitResult HitData(ForceInit);

	//Draws the green line
	DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Green, true, -1.0f, 0, 1.0f);

	//Gets the information and puts it in HitData
	GetWorld()->LineTraceSingleByChannel(HitData, StartTrace, EndTrace, CollisionChannel, TraceParams);

	//Does things with HitData's info (Checks if actor has a tag)
	if(HitData.GetActor() != NULL)
	{
		//Check if the item is to be picked up
		bool isInventoryItem = HitData.GetActor()->ActorHasTag("inventoryItem");

		if(isInventoryItem)
		{
			GEngine->AddOnScreenDebugMessage(9, 1.0f, FColor::Green, TEXT("Tag: isInventoryItem"));
			HitData.GetActor()->Destroy(false, true);
		}
		else
		{
			GEngine->AddOnScreenDebugMessage(9, 1.0f, FColor::Green, TEXT("Tag: n/a"));
		}
	}
	else if(HitData.GetActor() == NULL)
	{
		GEngine->AddOnScreenDebugMessage(9, 1.0f, FColor::Green, TEXT("NO HIT"));
	}
}
